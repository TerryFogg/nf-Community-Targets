...
void thread_0_entry(ULONG thread_input)
{
    ...
    nx_dhcp_create(&dhcp_client, &ip_0, (char *)"dhcp_client");
...
void g_tcp_sck_receive_cb(NX_TCP_SOCKET *socket_ptr)
{
    ...
    printf(
        "Received %d data bytes on local port: %d\n",
        p_packet->nx_packet_length,
    ...
    /* append data */
    nx_packet_data_append(p_packet, (char *)"Response: ", 10, &pool_0, NX_NO_WAIT);
