...
#ifdef NX_ENABLE_VLAN
            /* Add Ethernet header.  */
            if (nx_link_ethernet_header_add(
                    ip_ptr,
...
void _nx_ram_network_driver_output(NX_PACKET *packet_ptr, UINT interface_instance_id)
{
    ...
    for (i = 0; i < NX_MAX_RAM_INTERFACES; i++)
    {
        ...
        if (((destination_address_msw == ((ULONG)0x0000FFFF)) &&
             (destination_address_lsw == ((ULONG)0xFFFFFFFF))) || /* Broadcast match */
            ...
            ((destination_address_msw == 0) && (destination_address_lsw == 0)))
        {
            ...
            if (nx_packet_copy(packet_ptr, &packet_copy, next_ip->nx_ip_default_packet_pool, NX_NO_WAIT))
            {
                ...
                nx_link_packet_transmitted(
                    nx_ram_driver[interface_instance_id].nx_ram_driver_ip_ptr,
            ...
            /*lint -e{644} suppress variable might not be initialized, since "packet_copy" was initialized in
             * nx_packet_copy. */
    ...
    /* Restore preemption.  */
    /*lint -e{644} suppress variable might not be initialized, since "old_threshold" was initialized in previous
...
{
#ifdef NX_ENABLE_VLAN
    nx_link_ethernet_packet_received(
        ip_ptr,
        nx_ram_driver[interface_instance_id].nx_ram_driver_interface_ptr->nx_interface_index,
        packet_ptr,
        NX_NULL);
    ...
    packet_ptr->nx_packet_address.nx_packet_interface_ptr =
        nx_ram_driver[interface_instance_id].nx_ram_driver_interface_ptr;
