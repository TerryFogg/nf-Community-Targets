...
__nfweak void InitHardwareRTC(uint32_t secondsTime,int optional)
{
    (void)secondsTime;
    (void)optional;
    // Place holder for a call to underlying calue
}

__nfweak void GetHardwareRTC(uint32_t *secondsTime, uint32_t *subsec)
{
    (void)secondsTime;
    (void)subsec;
    // Place holder for a call to underlying calue
...
{
    SYSTEMTIME st;
    uint32_t secondsTime, subsec;

    // get the time from RTC
    GetHardwareRTC (&secondsTime, &subsec);

    // Convert from Unix time(years since 1970) to SYSTEMTIME(Years since 1601)
    int64_t time = ((int64_t)secondsTime * (int64_t)TIME_CONVERSION__TO_SECONDS) + TIME_UNIX_EPOCH_AS_TICKS;
...
{
    // Convert from SYSTEMTIME(Years since 1601) to Unix time(years since 1970)
    uint32_t secondsTime = (uint32_t)((utcTime - TIME_UNIX_EPOCH_AS_TICKS) / (int64_t)TIME_CONVERSION__TO_SECONDS);

    InitHardwareRTC(secondsTime, 0);
}
