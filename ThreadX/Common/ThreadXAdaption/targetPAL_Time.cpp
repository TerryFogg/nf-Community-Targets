...
{
    if (!(compareValueTicks == HAL_COMPLETION_IDLE_VALUE))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // HAL_COMPLETION::DequeueAndExec() need to subtract the current system time to set when the timer will
                // fire compareValueTicks is in CMSIS ticks (which equals to ms)
