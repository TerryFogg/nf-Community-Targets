...
//
//    screenX = pArgs[0].DereferenceArray();
//    FAULT_ON_NULL(screenX);
//    screenY = pArgs[1].DereferenceArray();
//    FAULT_ON_NULL(screenY);
//    uncalX = pArgs[2].DereferenceArray();
//    FAULT_ON_NULL(uncalX);
...
HRESULT Library_nanoFramework_Graphics_nanoFramework_UI_TouchPanel::GetCalibrationPointCount___VOID__BYREF_I4(
...
//    hbY.SetInteger(tcp.CalibrationY);
//    NANOCLR_CHECK_HRESULT(hbY.StoreToReference(stack.Arg3(), 0));
...
HRESULT Library_nanoFramework_Graphics_nanoFramework_UI_TouchPanel::GetCalibrationPoint___VOID__I4__BYREF_I4__BYREF_I4(
    CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
