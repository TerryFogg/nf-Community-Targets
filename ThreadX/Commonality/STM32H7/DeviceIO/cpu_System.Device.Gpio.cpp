...
bool EnablePinInterrupt(GPIO_PIN pinNumber, GPIO_INT_EDGE edge);
bool DisablePinInterrupt(GPIO_PIN pinNumber);
void SetPinUnused(GPIO_PIN pinNumber);
...
void UnlinkInputState(gpio_input_state *pState)
{
    ...
    DisablePinInterrupt(pState->pinNumber);
    pState->Unlink();
    SetPinUnused(pState->pinNumber);
    ...
    PinMode driveMode)
{
    ...
    if (pinISR != NULL && (pState->isrPtr == NULL))
    {
        ...
        EnablePinInterrupt(pState->pinNumber, intEdge);
...
void SetPinUnused(GPIO_PIN pinNumber)
{
  // Set to Push-pull with Data 0.
  // This maintains a static level with zero consumption and is the best reliable mode for EMI and EMS

   // Change this to implement 
   // • Output push - pull with pull - up or pull - down capability
 
    
    CPU_GPIO_SetDriveMode(pinNumber, PinMode_Output);
