...
int check_elf32_ph_entries(
    const std::vector<elf32_ph_entry> &entries,
    const address_ranges &valid_ranges,
    ...
    for (const auto &entry : entries)
    {
        if (entry.type == PT_LOAD && entry.memsz)
        {
            ...
            if (mapped_size)
            {
                ...
                if (rc)
                    return rc;
                // we don't download uninitialized, generally it is BSS and should be zero-ed by crt0.S, or it may be
                // COPY areas which are undefined
...
int elf2uf2(FILE *in, FILE *out)
{
    ...
    if (ram_style)
    {
        ...
        for (auto &page_entry : pages)
        {
                ...
                (page_entry.first < expected_ep_main_ram))
            {
                ...
                ((page_entry.first >= XIP_SRAM_START) && (page_entry.first < XIP_SRAM_END)) &&
    ...
    else
    {
        ...
        {
            for (uint32_t page = sector * FLASH_SECTOR_ERASE_SIZE; page < (sector + 1) * FLASH_SECTOR_ERASE_SIZE;
                 page += PAGE_SIZE)
            {
                if (page < last_page)
                {
