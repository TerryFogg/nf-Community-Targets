...
void Initialize_Board_LEDS_And_Buttons()
{
    ...
    LL_GPIO_Init(LD1_PORT, &gpio_InitStruct);

    gpio_InitStruct.Pin = LD2_PIN;
    LL_GPIO_Init(LD2_PORT, &gpio_InitStruct);

    gpio_InitStruct.Pin = LD3_PIN;
    LL_GPIO_Init(LD3_PORT, &gpio_InitStruct);

    // Turn them off
    LL_GPIO_SetOutputPin(LD1_PORT, LD1_PIN);
    LL_GPIO_SetOutputPin(LD2_PORT, LD2_PIN);
