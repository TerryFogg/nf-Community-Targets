...
typedef struct {
  uint32_t
      AudioMode; /*!< Specifies the SAI Block audio Mode.
  ...
  uint32_t MckOutput; /*!< Specifies whether master clock output will be
                         generated or not. This parameter can be a value of @ref
                         SAI_Block_MckOutput
                           @note This feature is only available on STM32H7xx
                         Rev.B and above */

  uint32_t
      OutputDrive; /*!< Specifies when SAI Block outputs are driven.
                        This parameter can be a value of @ref
                      SAI_Block_Output_Drive
                        @note This value has to be set before enabling the audio
                      block but after the audio block configuration. */

  uint32_t
      NoDivider; /*!< Specifies whether master clock will be divided or not.
                      This parameter can be a value of @ref SAI_Block_NoDivider
                      @note If bit NODIV in the SAI_xCR1 register is cleared,
                    the frame length should be aligned to a number equal to a
                    power of 2, from 8 to 256. If bit NODIV in the SAI_xCR1
                    register is set, the frame length can take any of the values
                    from 8 to 256.
                      @note The NODIV bit is the same as NOMCK bit in STM32H7xx
                    rev.Y */
  ...
  uint32_t Mckdiv; /*!< Specifies the master clock divider.
                      ...
                      Max_Data = 63.
                        @note This parameter is used only if AudioFrequency is
                      set to SAI_AUDIO_FREQUENCY_MCKDIV otherwise it is
                      internally computed. */
  ...
  /* This part of the structure is automatically filled if your are using the
     high level initialisation function HAL_SAI_InitProtocol */
  ...
  uint32_t ClockStrobing; /*!< Specifies the SAI Block clock strobing edge
                             sensitivity. This parameter can be a value of @ref
...
typedef struct {
  ...
  uint32_t FrameLength; /*!< Specifies the Frame length, the number of SCK
                           clocks for each audio frame. This parameter must be a
  ...
  uint32_t
      ActiveFrameLength; /*!< Specifies the Frame synchronization active level
                            length. This Parameter specifies the length in
                            number of bit clock (SCK + 1) of the active level of
                            FS signal in audio frame. This parameter must be a
...
typedef struct {
  ...
  uint32_t SlotActive; /*!< Specifies the slots in audio frame that will be
                          activated. This parameter can be a value of @ref
...
#define HAL_SAI_ERROR_AFSDET                                                   \
  0x00000004U /*!< Anticipated Frame synchronisation detection */
#define HAL_SAI_ERROR_LFSDET                                                   \
  0x00000008U /*!< Late Frame synchronisation detection */
...
#define SAI_ALAW_2CPL_COMPANDING                                               \
  (SAI_xCR2_COMP_1 | SAI_xCR2_COMP_0 | SAI_xCR2_CPL)
/**
 * @}
 */
...
#define __HAL_SAI_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__)                     \
  ((((__HANDLE__)->Instance->IMR & (__INTERRUPT__)) == (__INTERRUPT__))        \
       ? SET                                                                   \
       : RESET)
 ...
 *            @arg SAI_FLAG_AFSDET: Anticipated frame synchronization detection
 * flag.
 ...
 *            @arg SAI_FLAG_AFSDET: Clear Anticipated frame synchronization
 * detection
...
HAL_StatusTypeDef HAL_SAI_InitProtocol(SAI_HandleTypeDef *hsai,
                                       uint32_t protocol, uint32_t datasize,
                                       uint32_t nbslot);
