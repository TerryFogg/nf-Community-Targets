...
void Events_SetBoolTimer(bool *timerCompleteFlag, uint32_t millisecondsFromNow)
{
    if (timerCompleteFlag != NULL)
    {
        ...
        volatile uint64_t ticks = CPU_MillisecondsToTicks(millisecondsFromNow);
        tx_timer_change(&boolEventsTimer, ticks, 0);
        tx_timer_activate(&boolEventsTimer);
    }
}

uint32_t Events_WaitForEvents(uint32_t powerLevel, uint32_t wakeupSystemEvents, volatile uint32_t timeoutMilliseconds)
{
