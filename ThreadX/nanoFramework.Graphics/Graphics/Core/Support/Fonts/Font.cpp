...
void CLR_GFX_Font::Font_Initialize()
{
    ...
    m_ranges = (CLR_GFX_FontCharacterRange *)buf;
    buf += m_font.GetRangeSize();
    m_chars = (CLR_GFX_FontCharacter *)buf;
    buf += m_font.GetCharacterSize();
    if (m_font.m_flags & CLR_GFX_FontDescription::c_FontEx)
    {
        m_rangesEx = (CLR_GFX_FontCharacterRangeEx *)buf;
        buf += m_font.GetRangeExSize();
        m_charsEx = (CLR_GFX_FontCharacterEx *)buf;
        buf += m_font.GetCharacterExSize();
        m_antiAliasingData = buf;
        buf += m_fontEx.GetAntiAliasSize();
    }
...
HRESULT CLR_GFX_Font::CreateInstance(CLR_RT_HeapBlock &ref, const CLR_UINT8 *data, CLR_RT_Assembly *assm)
{
    ...
    fd = (const CLR_GFX_FontDescription *)data;
    data += sizeof(CLR_GFX_FontDescription);
    ...
    font->m_ranges = (CLR_GFX_FontCharacterRange *)data;
    data += fd->GetRangeSize();
    font->m_chars = (CLR_GFX_FontCharacter *)data;
    data += fd->GetCharacterSize();
    font->m_bitmap.m_palBitmap.data = (CLR_UINT32 *)data;
    data += bm->GetTotalSize();

    font->m_bitmap.m_bm.m_flags |= CLR_GFX_BitmapDescription::c_ReadOnly;

    if (fd->m_flags & CLR_GFX_FontDescription::c_FontEx)
    {
        font->m_fontEx = *((const CLR_GFX_FontDescriptionEx *)data);
        data += sizeof(CLR_GFX_FontDescriptionEx);
        font->m_rangesEx = (CLR_GFX_FontCharacterRangeEx *)data;
        data += fd->GetRangeExSize();
        font->m_charsEx = (CLR_GFX_FontCharacterEx *)data;
        data += fd->GetCharacterExSize();
...
CLR_UINT32 CLR_GFX_Font::DrawCharHelper(int x, int y, CLR_UINT32 flags, CLR_UINT16 &opacity, void *param)
{
...
void CLR_GFX_Font::CountCharactersInWidth(
    LPCSTR str,
    int maxChars,
    int width,
    int &totWidth,
    ...
    while (charLimit-- > 0)
    {
        ...
        if (fNewLine || chrWidth > width)
        {
            if (fWordWrap)
            {
                ...
                if (breakPoint)
                {
                    if ((fNewLine && lastChar == ' ') || (!fNewLine && c != ' '))
                    {
                        totWidth = breakWidth;
                        str = breakPoint;
                        num = breakIndex;
...
int GetCharInfoCmp(const void *c, const void *r)
{
    ...
    if ((*chr) < range->m_firstChar)
        return -1;
    if ((*chr) > range->m_lastChar)
        return 1;
    /******************************/ return 0;
}
...
void CLR_GFX_Font::GetCharInfo(CLR_UINT16 c, CLR_GFX_FontCharacterInfo &chrEx)
{
    ...
    range = (CLR_GFX_FontCharacterRange *)
        bsearch(&c, m_ranges, m_font.m_ranges, sizeof(CLR_GFX_FontCharacterRange), GetCharInfoCmp);
    ...
    if (range)
    {
        ...
        if (chr)
        {
            ...
            chrEx.iAntiAlias = (this->m_font.m_flags & CLR_GFX_FontDescription::c_AntiAliasMask) >>
                               CLR_GFX_FontDescription::c_AntiAliasShift;

            if (chrEx.iAntiAlias > 1 &&
                m_charsEx[chrIndex].m_offsetAntiAlias != CLR_GFX_FontCharacterEx::c_offsetNoAntiAlias)
            {
                int iRange = (int)(range - m_ranges);
                chrEx.antiAlias = m_antiAliasingData + this->m_rangesEx[iRange].m_offsetAntiAlias +
                                  m_charsEx[chrIndex].m_offsetAntiAlias;
            }
            else
            {
